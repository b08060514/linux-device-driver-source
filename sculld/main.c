...
{
	struct async_work *stuff = container_of((struct delayed_work *)p,struct async_work, work);
...
static ssize_t sculld_aio_read(struct kiocb *iocb,const struct iovec *iov, unsigned long nr_regs,
		loff_t pos)
{
	return sculld_defer_op(0, iocb, iov->iov_base, iov->iov_len, pos);
}

static ssize_t sculld_aio_write(struct kiocb *iocb,const struct iovec *iov, unsigned long nr_regs,
		loff_t pos)
{
	return sculld_defer_op(1, iocb, iov->iov_base, iov->iov_len, pos);
}
...
static ssize_t sculld_show_dev(struct device *ddev,struct device_attribute *attr, char *buf)
{
	...
	struct sculld_dev *dev = container_of(container_of(ddev,struct ldd_device, dev), struct sculld_dev, ldev);
